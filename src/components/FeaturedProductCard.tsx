...
export default function FeaturedProductCard({ product }: { product: any }) {
  // Handle both static and dynamic product formats
  ...
  if (product.prices) {
    if (Array.isArray(product.prices)) {
      startingPrice = Math.min(...product.prices.map((p: any) => p.price));
    } else if (typeof product.prices === 'object') {
     const priceValues = Object.values(product.prices).filter((v) => typeof v === "number") as number[];
     if (priceValues.length > 0) {
       startingPrice = Math.min(...priceValues);
     }
    }
        ...
        <div className="absolute top-6 right-6">
          <div className="bg-black/40 backdrop-blur-md border border-white/10 px-4 py-2 rounded-full transform group-hover:-translate-y-1 transition-transform duration-500">
            <span className="text-primary text-sm font-bold tracking-wider">
              {startingPrice > 0 ? `ab ${startingPrice.toFixed(2)} €` : "Preis auf Anfrage"}
            </span>
          </div>
        </div>
      </div>
