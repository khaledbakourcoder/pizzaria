        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 backdrop-blur-sm p-0 md:p-4">
            ...
            <div className="relative w-full max-w-lg bg-background max-h-[90vh] rounded-t-[2rem] md:rounded-[2rem] border-t md:border border-border shadow-2xl flex flex-col animate-slide-up overflow-hidden">
                ...
                <div className="px-6 py-4 flex justify-between items-center border-b border-border">
                    <div>
                        <h4 className="text-xl font-serif italic text-foreground">{productName}</h4>
                        <p className="text-[10px] text-muted uppercase tracking-widest">Konfiguration</p>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 bg-surface rounded-full flex items-center justify-center text-foreground hover:bg-white/10 transition-colors">✕</button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
                    ...
                    <div>
                        <p className="text-[10px] text-muted font-bold uppercase tracking-widest mb-4">Größe wählen:</p>
                        <div className="grid grid-cols-3 gap-2">
                            {Object.entries(productPrices).map(([size, price]) => (
                                ...
                                >
                                    ...
                                    <span className="block opacity-60">{(price as number).toFixed(2)} €</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    {categoryData?.doughs && categoryData.doughs.length > 0 && (
                        <div>
                            <p className="text-[10px] text-muted font-bold uppercase tracking-widest mb-4">Teigart:</p>
                            <div className="space-y-2">
                                {categoryData.doughs.map((dough: any) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs">{dough.price > 0 ? `+${dough.price.toFixed(2)} €` : 'Inklusive'}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                    ...
                    {categoryData?.toppings && categoryData.toppings.length > 0 && (
                        <div>
                            <p className="text-[10px] text-muted font-bold uppercase tracking-widest mb-4">Extras & Zusätze:</p>
                ...
                <div className="p-6 bg-surface border-t border-border flex items-center justify-between gap-4">
                    <div>
                        <p className="text-[9px] text-muted uppercase tracking-widest font-bold">Gesamt</p>
                        <p className="text-2xl font-bold text-foreground leading-none">{total} €</p>
                    </div>
                    <button disabled className="px-8 py-3 bg-white/5 border border-border text-muted rounded-xl text-xs font-bold cursor-not-allowed italic">
                        Bald verfügbar
                    </button>
                </div>
            </div>
